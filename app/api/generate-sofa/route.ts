...
2. 如果房间参考图里已有沙发/座椅，先判断原座位区是否是最合理落点。若是，产品沙发替换原座位；若不是，可选择更合理空位，但必须说明并遵守真实会客关系、朝向、离茶几/电视/墙面的距离和通行动线。
3. 再选择当前景别对应的相机机位：${shotName}。只允许移动相机、改变焦段、机位高度、景深和裁切；拍摄角度可以正面、侧面、背侧或斜侧自适应，不固定。
4. 最后融合产品沙发本体。不要为了画面居中、商品正面角度、近景放大或构图美观，把沙发重新放到原座位前方、窗前、房间中央、门洞、通道、电视所在墙同侧、电视下方、电视旁边贴墙区或固定柜体前。
5. 如果窗前是主要采光面或落地窗/窗帘区域，窗前空地不是默认沙发区；除非落位方案明确允许，否则必须保留采光和通行。
6. 如果房间有电视/媒体墙，沙发必须落在电视对侧或斜对侧的观看区，主坐面朝向电视/媒体墙；禁止把沙发放在电视所在墙、电视下方或与电视同侧贴墙。
7. 如果电视/媒体墙在画面左侧墙，沙发应优先落在画面右半部或右下/下方开阔地并朝左；如果电视/媒体墙在画面右侧墙，沙发应优先落在画面左半部或左下/下方开阔地并朝右；如果电视/媒体墙在后墙，沙发应优先落在画面下方前景可用地面并朝后墙。
8. 绝对禁止结果：产品沙发被随意放在前景/房间中央/原沙发前方，导致挡通道、挡视线、和原家具拥挤冲突。保留原家具本身不是错误，错误是产品沙发没有合理功能位置。
9. 绝对禁止结果：为了电商主图把沙发实际坐向转成朝镜头，导致它没有面向电视/茶几/会客中心。镜头可以动，沙发坐向不能为了镜头乱转。
...
export async function POST(req: Request) {
  try {
    ...
    let modelUsed;
    let infoText;
    const placementHint = '不使用任何前端预览坐标或默认锚点；必须完全根据房间参考图自动识别最合理座位区。如果原房间已有沙发/座椅，先评估原座位区是否最合理：最合理则替换原座位区；如果有更合理空位，可以不替换，但必须保持真实会客关系、朝向和动线；有电视/媒体墙时必须在电视对侧或斜对侧，禁止电视同侧贴墙和电视下方。';
    ...
    let placementPlan = '空间落位分析未单独返回；请按房间参考图中的真实地面、原座位区、会客区、窗户、门洞、固定柜体、电视/媒体墙和主要通道，选择最稳定且不挡光不挡路的可摆放区域；如果已有原沙发/座椅，先评估原座位区是否最合理，最合理则替换，不是最合理则选择更合理空位并保证不拥挤不挡路；有电视/媒体墙时必须在电视对侧或斜对侧，不能在电视所在墙同侧或电视下方。';
    try {
