...
export async function POST(req: Request) {
  try {
    ...
    if (roomData) {
      parts.push({ text: '【房间/场景参考图 - 空间结构来源】只用于锁定真实房间构造、地面墙面、窗户、门洞、电视/媒体墙、固定柜体、吊顶、光线方向、镜头透视、原座位区和整体氛围；不要用房间图里的原沙发款式、抱枕、文字或装饰改变商品沙发。' });
...
7. 如果房间参考图已有沙发、座椅、贵妃位或明确会客座位，最高优先级是用产品沙发替换原座位区，保持原来的占地、朝向、与茶几/地毯/电视/通道的关系；不能因为窗前、房间中央或画面中心有空地就重新选择位置。
...
11. 如果电视/媒体墙在画面左侧墙，沙发应优先落在画面右半部或右下/下方开阔地并朝左；如果电视/媒体墙在画面右侧墙，沙发应优先落在画面左半部或左下/下方开阔地并朝右；如果电视/媒体墙在后墙，沙发应优先落在画面下方前景可用地面并朝后墙。
12. 远景/中景/近景共用同一个物理落位，通过相机距离、焦段、机位高度、景深和裁切区分；拍摄角度不固定，可以正面、侧面、背侧或斜侧自适应，但不要为了角度改变沙发落位或房间构造。
13. 近景是靠近已落位沙发拍局部材质和结构，可以自然裁切，不是把完整沙发整体巨大化后塞进窗前或房间中央。
14. 沙发必须贴合地面透视，底座/脚部有明确接触点、接触阴影和投射阴影；光影、比例、材质质感真实。
15. 如果用户只用文字描述，就按文字生成高端家居电商摄影图，构图干净、主体明确、可直接用于商品展示。
`,
    });
