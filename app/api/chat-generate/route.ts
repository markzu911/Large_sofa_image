...
${placementPlan ? `【本次房间落位方案 - 必须优先执行】\n${placementPlan}\n\n执行要求：最终画面必须遵守上面的唯一锁定落点。远景/中景/近景只能通过相机距离、焦段、高度、景深和裁切变化实现，不能重新选择沙发位置，不能把沙发挪到窗前、门洞、通道、电视所在墙同侧、电视下方、电视旁边贴墙区、固定柜体前或房间中央。\n` : ''}
...
5. 如果房间参考图已有沙发、座椅、贵妃位或明确会客座位，最高优先级是用产品沙发替换原座位区，保持原来的占地、朝向、与茶几/地毯/电视/通道的关系；不能因为窗前、房间中央或画面中心有空地就重新选择位置。
6. 有本次落位方案时，沙发位置、朝向、禁放区和机位以该方案为准；没有方案时，也必须按真实室内设计逻辑选择稳定可放区，不能随机居中、挡窗、挡门、挡电视墙、遮挡动线或悬浮。
7. 如果窗前是主要采光面或落地窗/窗帘区域，窗前空地不是默认沙发区；除非落位方案明确允许，否则必须保留采光和通行。
8. 如果房间有电视/媒体墙，沙发必须落在电视对侧或斜对侧的观看区，主坐面朝向电视/媒体墙；禁止把沙发放在电视所在墙、电视下方或与电视同侧贴墙。
9. 如果电视/媒体墙在画面左侧墙，沙发应优先落在画面右半部或右下/下方开阔地并朝左；如果电视/媒体墙在画面右侧墙，沙发应优先落在画面左半部或左下/下方开阔地并朝右；如果电视/媒体墙在后墙，沙发应优先落在画面下方前景可用地面并朝后墙。
10. 远景/中景/近景共用同一个物理落位，通过相机距离、焦段、机位高度、景深和裁切区分；拍摄角度不固定，可以正面、侧面、背侧或斜侧自适应，但不要为了角度改变沙发落位或房间构造。
11. 近景是靠近已落位沙发拍局部材质和结构，可以自然裁切，不是把完整沙发整体巨大化后塞进窗前或房间中央。
12. 沙发必须贴合地面透视，底座/脚部有明确接触点、接触阴影和投射阴影；光影、比例、材质质感真实。
13. 如果用户只用文字描述，就按文字生成高端家居电商摄影图，构图干净、主体明确、可直接用于商品展示。
`,
    });
