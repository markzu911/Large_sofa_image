...
3. 可替换对象：房间里哪些原有可移动家具可以被商品沙发替换或移走；哪些结构和固定家具绝对不能动。
4. 禁入边界：逐条列出本图不能放沙发的区域，尤其是窗户/窗帘主体前、门洞、通道、电视/媒体墙正前方、固定柜体前、墙体开口、过窄或非地面的区域。
5. 唯一锁定落点：只给一个最终落点。必须同时写：
...
6. 远景机位：相机如何后退展示空间，沙发占比小但仍在同一锁定落点。
7. 中景机位：相机如何靠近成为电商主图，沙发占比适中但仍在同一锁定落点。
8. 近景机位：相机如何靠近已落位沙发拍扶手、坐垫、靠背、缝线和材质；可以自然裁切局部，但不能把完整沙发巨大化后重新摆到窗前/房间中央/通道。
9. 绝对禁止的错误结果：明确写出本图最容易出错的 3-5 种摆法。

决策规则：
- 先锁定不可移动结构，再找真实地面可放区，最后决定沙发落点和机位；不能先把沙发居中再反推房间。
- 沙发必须落在地面可承载区域，不能占用窗户/窗帘主体、门洞、主要通道、电视/媒体墙正前方、固定柜体前、墙体开口或明显过窄区域。
...
- 如果房间有电视/媒体墙/壁炉墙/主观看墙，沙发主坐面应朝向它或斜向它；不能面向窗户而背离观看墙。
