...
export async function POST(req: Request) {
  try {
    ...
    if (productData) {
      ...
      parts.push({ text: '【沙发/商品参考图 - 商品身份最高优先级】必须复刻这同一件沙发本体。先在脑中抠出沙发本体，只提取沙发的轮廓、结构、模块数量、扶手/靠背/坐垫形态、缝线、扣点、褶皱、材质纹理、原本颜色和真实固定附属物；这不是风格参考，禁止生成相似款或改款。除非商品图中清晰固定在沙发本体上，否则严禁新增任何文字、Logo、品牌名、商标、图案、刺绣、标签、抱枕、靠包、毯子或装饰。必须忽略并禁止复制这张图里的房间背景、墙面文字、墙柜/柜体、灯具、茶几、玩偶/公仔、地毯、餐桌、画作、窗帘、绿植、水印和所有其他非沙发物体。' });
...
${placementGuideImage ? `【自动落位遮罩图规则】\n输入中包含一张内部空间坐标遮罩图：红色是绝对禁放区，绿色是沙发主体目标落位区，绿色箭头是主坐面/开口侧朝向。最终图必须遵守遮罩，但不要渲染遮罩颜色、箭头、边框或辅助图痕迹。\n` : ''}
...
11. 如果电视/媒体墙在画面左侧墙，电视所在左半区、电视下方、电视旁边同墙贴墙区全部禁放；除非房间透视明确相反，沙发中心必须避开 X<50% 的电视同侧区，优先落在画面右半部或右下/中下观看区，主坐面朝左/左上看电视。
12. 如果电视/媒体墙在画面右侧墙，电视所在右半区、电视下方、电视旁边同墙贴墙区全部禁放；除非房间透视明确相反，沙发中心必须避开 X>50% 的电视同侧区，优先落在画面左半部或左下/中下观看区，主坐面朝右/右上看电视。
...
23. 如果用户只用文字描述，就按文字生成高端家居电商摄影图，构图干净、主体明确、可直接用于商品展示。
`,
    });
