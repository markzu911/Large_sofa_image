...
const SHOT_PRESETS = [
  {
    ...
    name: '远景：空间全貌',
    height: '自适应站立机位',
    angle: '沿房间参考图原相机方向的广角空间镜头，不改变墙体方位和电视位置',
    description: '保持房间参考图原视角，完整看空间结构和真实落位。',
    promptGuide: '远景：保持房间参考图原相机方向和墙体方位，广角展示完整空间；不能把左墙电视变成后墙电视，沙发物理坐向必须朝向电视/茶几/会客中心，沙发占比小且落位合理。'
  },
  {
    id: 'medium',
    name: '中景：电商主图',
    height: '自适应视平机位',
    angle: '沿房间参考图原相机方向推近的标准主图镜头',
    description: '在原房间视角上靠近合理落位后的沙发，主体清楚，仍保留地面接触和背景。',
    promptGuide: '中景：在房间参考图原相机方向上推近到主图距离，电视、窗户、墙体方位不能变化；沙发是主体，但只能移动相机/裁切，不能为了正面展示把沙发旋转成朝镜头、拉到前景或放到房间中央。'
  },
  {
    id: 'close',
    name: '近景：材质细节',
    height: '自适应近摄机位',
    angle: '沿房间参考图原相机方向推近裁切的中长焦局部细节镜头',
    description: '相机靠近已合理落位的沙发，裁切局部材质、扶手、坐垫和缝线。',
    promptGuide: '近景：保持房间参考图原相机方向，只在正确落位后推近裁切局部材质、扶手、坐垫、靠背、缝线和接触阴影；电视、窗户、墙体方位不能变化，最终画面不能出现完整沙发全貌，不能把完整沙发拉到镜头前重新摆放。'
  }
] as const;
...
const readApiResponse = async (response: Response, fallbackLabel: string) => {
  if (response.status === 413) {
    return {
      success: false,
      errorMessage: `${fallbackLabel}请求体过大。已改用后端保存链路，请重新生成；如果仍出现，请降低清晰度或减少参考图尺寸。`,
    };
  }
...
export default function App() {
  ...
  const handleSynthesis = async () => {
    ...
    try {
      const requestPayload = {
        ...
        customPrompt: `${currentPreset.promptGuide} 高清还原等级: ${resolution}。房间参考图是最终画面的原始底片，必须保持原相机方向、电视所在墙、窗户、边柜、墙角和地面透视；禁止把左墙电视变成后墙电视或重构房间角度。商品沙发必须作为同一件产品100%还原，不能改款、改结构、加抱枕、加文字、加Logo。生成前只抠取产品图里的沙发本体，产品图中的茶几、玩偶/公仔、灯、柜体、地毯、餐桌、画作、窗帘、绿植、墙面、文字和水印都不是产品，也不能复制到房间或影响落位。不使用前端预览坐标，必须自动判断最合理的沙发座位区；如果房间原图已有沙发或座椅，先判断原座位区是否最合理，最合理则替换，否则选择更合理空位并保证不拥挤不挡路。电视在左墙则沙发主体必须避开左侧58%电视同侧禁放区，落在右半区/右下/中下观看区且朝左；电视在右墙则避开右侧58%禁放区并落在左半区/左下/中下观看区且朝右；电视在后墙则在下方观看区并朝后墙；禁止电视同侧、电视下方和电视旁同墙摆放。拍摄角度只能在原房间视角上推近、裁切、微调焦段，不能为了商品正面改变房间视角或沙发物理坐向。`
      };
  ...
  const runChatGeneration = async (promptOverride?: string) => {
    ...
    try {
      ...
      if (data.success && data.image) {
        const initialNote = data.saveScheduled
          ? '图片已生成，可预览和下载；后端正在继续保存到 SaaS 图片库。'
          : data.needsSaasSave || data.savedToSaas === false
        ...
        updateChatMessage(loadingId, {
          ...
          generation: {
            ...
            resolution: chatResolution,
            note: initialNote,
            saveScheduled: Boolean(data.saveScheduled),
            needsSaasSave: !data.saveScheduled && (data.needsSaasSave || data.savedToSaas === false),
